                    ...
                    if (is_array($serials) && count($serials)) {
                        foreach ($serials as $serial) {
                            if ($serial->is_return) {
                                ...
                                $checkSalesSerial = $this->db->query("
                                    select
                                        ps_serial_number 
                                    ...
                                    and  ps_brunch_id = ?
                                    and ps_status = 'a'     
                                ", [$serial->ps_serial_number, $this->session->userdata('BRANCHid')]);

                                if ($checkSalesSerial->num_rows() > 0) {
                                    $res = ['success' => false, 'message' => 'This ' . $serial->ps_serial_number . ' number already sales. Purchase return is not possible'];
                                    echo json_encode($res);
                                    exit;
                                }
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                $purchaseDetails = array(
                    ...
                    'warranty_month'                => $product->warrantyMonth,
                    'Status'                        => 'a',
                    'AddBy'                         => $this->session->userdata("FullName"),
                    'AddTime'                       => date('Y-m-d H:i:s'),
                ...
                $serialArr = [];
                if (count($product->SerialCartStore) > 0) {
                    foreach ($product->SerialCartStore as $item) {
                        $data = array(
                            'ps_prod_id'          => $item->productId,
                            'ps_serial_number'    => $item->serialNumber,
                            ...
                            'purchase_total'      => $item->purchaseRate,
                ...
                if ($inventoryCount == 0) {
                    ...
                    $this->db->query("UPDATE tbl_currentinventory SET purchase_quantity = purchase_quantity + ? WHERE product_id = ? AND branch_id = ?
                ...
                $this->db->query("UPDATE tbl_product SET Product_Purchase_Rate = (((Product_Purchase_Rate * ?) + ?) / ?), 
                    Product_SellingPrice = ? WHERE Product_SlNo = ?
                ", [$previousStock, $product->total, ($previousStock + $product->quantity), $product->salesRate, $product->productId]);
            }

...
class Sales extends CI_Controller
{
    ...
    public function Quotation($q_id)
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        ...
        $this->cart->destroy();
        $this->session->unset_userdata('cheque');
        $data['title'] = "Product Sales from Quotation";

        $invoice = $this->mt->generateSalesInvoice();

        $data['isService'] = 'false';
        $data['salesId'] = 0;
        $data['quotationId'] = $q_id;
        $data['isPosSales'] = 0;
        $data['invoice'] = $invoice;
        $data['content'] = $this->load->view('Administrator/sales/product_sales', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    ...
    public function getSerialReport()
    {
        ...
        $res['saleReport'] =  $this->db->query("
            ...
            p.Product_Name,
            b.Brunch_name,
            DATE_ADD(sm.SaleMaster_SaleDate, INTERVAL (select p.warranty) month ) as warranty_date
...
<?php
    ...
    public function getSecquences()
    {
        $data = json_decode($this->input->raw_input_stream);

        $getSerials = $this->db->query("
            select
                ps.ps_id,
                ps.ps_prod_id,
                ps.ps_serial_number,
            ...
            join tbl_product p on p.Product_SlNo = ps.ps_prod_id 
            where ps.ps_id >= ?
            and ps.ps_prod_id = ?
            and ps.ps_s_status = 'no'
            and ps.ps_purchase_inv = ?
            and ps.ps_brunch_id = ?
            limit ?
        ", [$data->id, $data->productId, $data->invoice, $this->session->userdata('BRANCHid'), (int) $data->limit])->result();

        echo json_encode($getSerials);
    }
    ...
    public function order()
    {
        ...
        $data['isPosSales'] = 0;
        $data['invoice'] = $invoice;
        $data['content'] = $this->load->view('Administrator/sales/order', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function orderRecord()
    {
        ...
        $data['content'] = $this->load->view('Administrator/sales/order_record', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function orderApprove()
    {
        ...
        try {
            ...
            $sale = $this->db->query("
                ...
                where sm.SaleMaster_SlNo = ?
                and sm.SaleMaster_branchid = ?
            ", [$saleId, $this->session->userdata('BRANCHid')])->row();
            ...
            foreach ($saleDetails as $product) {
                ...
                if ($product->SaleDetails_TotalQuantity > $stock) {
                    $res->success = false;
                    $res->message = $product->Product_Name . ' stock unavailable. Please purchase first';
                    echo json_encode($res);
                    exit;
                }
            }
            ...
            foreach ($saleDetails as $product) {
                ...
                $this->db->query("
                    ...
                    set ps_s_status = 'yes'
                    where ps_s_status = 'p'
                    and sales_details_id = ?
                    and ps_prod_id = ?
                    and ps_brunch_id = ?
                ", [$product->SaleDetails_SlNo, $product->Product_IDNo, $this->session->userdata('BRANCHid')]);
            }
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                $res->success = true;
                $res->message = 'Order approved successfully';
            }
        } catch (\Exception $e) {
            $this->db->trans_rollback();
            $res->success = false;
            $res->message = 'Order approved failed' . $e->getMessage();
        }
    ...
    public function editOrder($orderId)
    {
        ...
        $sales = $this->db->query("select * from tbl_salesmaster where SaleMaster_SlNo = ?", $orderId)->row();
        $data['isService'] = 'false';
        $data['salesId'] = $orderId;
        $data['isPosSales'] = 0;
        $data['invoice'] = $sales->SaleMaster_InvoiceNo;
        $data['content'] = $this->load->view('Administrator/sales/order', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    ...
    public function orderDelete()
    {
        ...
        try {
            ...
            /*Get Sale Details Data*/
            $saleDetails = $this->db->select('SaleDetails_SlNo, Product_IDNo')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();

...
class Products extends CI_Controller
{
    ...
    public function addProduct()
    {
        ...
        try {
            ...
            if (!empty($_FILES['image'])) {
                $pImg = explode('.', $_FILES['image']['name']);
                $product['Product_Img'] = $productObj->Product_Code . '.' . end($pImg);
                move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/products/' . $product['Product_Img']);
            }
    ...
    public function updateProduct()
    {
        ...
        // $productObj = json_decode($this->input->raw_input_stream);
        $productObj = json_decode($this->input->post('data'));
        // $productImage = $this->db->query("select * from tbl_product where Product_SlNo = ?", [$productObj->Product_SlNo])->row()->Product_Img;
        ...
        try {
            ...
            if ($productObj->is_serial == 0) {
                $checkPurchaseSerial = $this->db->query("select * from tbl_product_serial_numbers ps where ps.ps_status = 'a' and ps.ps_prod_id = ?", [$productObj->Product_SlNo]);

                if ($checkPurchaseSerial->num_rows() > 0) {
                    $res = ['success' => false, 'message' => 'Could not change is serial! This product already serial purchase used!'];
                    echo json_encode($res);
                    exit;
                }
            }
    ...
    public function getTotalStock()
    {
        ...
        if (isset($data->subCategoryId) && $data->subCategoryId != null) {
            $clauses .= " and p.subcategory_id = '$data->subCategoryId'";
        }
...
<?php
    ...
    public function checkProductSerial()
    {

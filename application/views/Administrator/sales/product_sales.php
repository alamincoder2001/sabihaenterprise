				...
				<div class="widget-main">
					...
					<div class="row">
						...
						<div class="col-sm-2">
							...
							<input type="password" ref="productPurchaseRate" v-model="selectedProduct.Product_Purchase_Rate" v-on:mousedown="toggleProductPurchaseRate" v-on:mouseup="toggleProductPurchaseRate" readonly title="Purchase rate (click & hold)" style="font-size:12px;width:100%;text-align: center;display:none">
											...
											<div class="form-group">
												<div class="col-xs-12">
													<div class="form-check">
														<input type="checkbox" v-model="sales.sendSMS" class="form-check-input" id="exampleCheck1">
														<label onMouseOver="this.style.cursor='pointer'" class="form-check-label" for="exampleCheck1" style="color: #ff0500;">Send SMS</label>
													</div>
												</div>
											</div>
										</td>
	...
	new Vue({
		...
		data() {
			return {
				...
				qId: parseInt('<?php echo $quotationId; ?>'),
				vatPercent: 0,
				discountPercent: 0,
				cart: [],
				employees: [],
		...
		methods: {
			...
			async getCustomerDue() {
				if (this.selectedCustomer.Customer_Type == 'wholesale') {
			...
			addToCart() {
				if (this.qId != 0) {
					if (this.selectedProduct.is_serial == false) {
						alert('Only Serial Item can be update')
						return;
					}
					let sItem = {
						ps_id: this.psId,
					...
					let cehckDuplicate = this.cart[getCurrentInd].SerialStore.findIndex((item) => {
						return (item.ps_serial_number == this.psSerialNumber);
					});

					if (parseInt(this.cart[getCurrentInd].quantity) == parseInt(this.cart[getCurrentInd].SerialStore.length)) {
						alert('Required Quantity Serial Item already added')
						return
					}
					...
					if (cehckDuplicate > -1) {
						...
						this.cart[getCurrentInd].SerialStore.push(sItem)
					}
					...
					if (this.serials.length > 0 && this.isProductFilterByBarcode && this.barcodeValue != '') {
						let filteredProducts = this.serials.filter(p => p.ps_serial_number == this.barcodeValue);
						let serial = {
							ps_id: filteredProducts[0].ps_id,
							ps_serial_number: filteredProducts[0].ps_serial_number
						}
						product.SerialStore = [];
						product.quantity = 1;
						product.total = parseFloat(product.salesRate * product.quantity);
						product.SerialStore.push(serial)
					...
					if (this.selectedProduct.is_serial == true && product.SerialStore.length == 0 && this.barcodeValue == '') {
						alert('Select serial number. This product only serial availabe');
						return;
					}

					if (this.serials.length && this.serial == null && this.sequences.length == 0 && this.barcodeValue == '') {
					...
					if (this.selectedProduct.is_serial == true && product.quantity > this.productStock && this.sales.isService == 'false') {
					...
					if (checkCart.length > 0) {
						checkCart.map((item) => {
							...
							if (storeObj.length && product.SerialStore.length) {
								...
								this.cart[getCurrentInd].quantity = this.selectedProduct.quantity;
								this.cart[getCurrentInd].salesRate = this.selectedProduct.Product_SellingPrice;
								this.cart[getCurrentInd].total = parseFloat(product.salesRate * product.quantity);
							}
						})
					} else {
			...
			removeFromCart(ind) {
				if (this.qId != 0) {
					alert('Quotation Sale cannot remove item.')
					return
				}
			...
			calculateTotal() {
				...
				if (this.sales.paid != 0 && this.sales.isPosSales != 0) {
				...
				axios.post(url, data).then(async res => {
					...
					if (r.success) {
						...
						if (conf) {
							...
							if (this.sales.isPosSales == 1) {
								window.location = this.sales.isService == 'false' ? '/pos_sales' : '/pos_sales';
							} else {
			...
			async getQuotations() {
				await axios.post('/get_quotations', {
					quotationId: this.qId
				}).then(res => {
					let r = res.data;
					let sales = r.quotations[0];
					this.sales.salesBy = sales.AddBy;
					this.sales.salesFrom = sales.SaleMaster_branchid;
					this.sales.salesDate = sales.SaleMaster_SaleDate;
					this.sales.salesType = sales.SaleMaster_SaleType;
					...
					// this.sales.paid = sales.SaleMaster_PaidAmount;
					// this.sales.previousDue = sales.SaleMaster_Previous_Due;
					this.sales.due = sales.SaleMaster_TotalSaleAmount;
					// this.sales.returnAmount = sales.return_amount;
					// this.sales.note = sales.SaleMaster_Description;
					// this.sales.serviceCharge = sales.service_charge;
					...
					this.selectedCustomer = {
						...
						Customer_Code: sales.Customer_Code,
						Customer_Name: sales.Customer_Name,
						display_name: sales.Customer_Code + ' - ' + sales.Customer_Name + ' - ' + sales.Customer_Mobile,
						Customer_Mobile: sales.Customer_Mobile,
						Customer_Address: sales.Customer_Address,
						Customer_Type: sales.Customer_Type
					}
